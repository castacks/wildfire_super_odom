...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", dest="config", help="JSON configuration file", required=True
    )
    parser.add_argument(
        "--mode",
        dest="mode",
        help="0 for SLAM, 1 for localization",
        choices=["0", "1"],
        required=True,
    )
    parser.add_argument(
        "--dataset-index",
        dest="index",
        help="(optional) index of the dataset in the JSON configuration file to run",
        type=int,
        required=False,
    )
    ...
    for i in range(len(configs)):
        ...
        assert (
            len(file_list) > 0
        ), "Number of bags in the data folder, \033[91m{}\033[0m should be non-zero".format(
            config["datapath"]
        )
        if len(file_list) >= 2:
        ...
        if localization_mode:
            shared_path = os.path.commonpath(
                [config["datapath"], config["cloudpath"], config["outputpath"]]
            )
            docker_datapath = "/media/drive/" + os.path.relpath(
                config["datapath"], shared_path
            )
            docker_cloudpath = "/media/drive/" + os.path.relpath(
                config["cloudpath"], shared_path
            )
            docker_outputpath = "/media/drive/" + os.path.relpath(
                config["outputpath"], shared_path
            )
        else:
        ...
        if i + 1 < len(configs):
            proc = Process(
                target=escape_func,
                args=[math.ceil(end_timestamp - start_timestamp) + 60],
            )
            proc.start()
        ...
        os.environ["PLAY_DURATION"] = (
            "65536"
            if config["start_time"] == -1
            else str(end_timestamp - start_timestamp)
        )
        if localization_mode:
            os.environ["DURATION"] = str(
                math.ceil(end_timestamp - start_timestamp) + 40
            )
